#include <raylib.h>
#include <random>
#include <cmath>
using namespace std;
float altura = 500;
float largura = 500;
//variavel aleatoria
random_device rd;
mt19937 gen(rd());
uniform_int_distribution<int> evento(0, 1);

Sound somPonto;
Sound somBatida;
Sound somBatida2;
Color azul = {0,0,135,250};
Color verde = {34, 110, 50, 255};
Color vermelho ={135,0,0,250};
Color nada = {0,0,0,0,};
Color branco = { 235, 235, 235, 255 };
Color preto ={0,0,0,20};

int frame = 0;
int evento_tempo = 150 * 7;
int aleatorio2 = 0;
int aleatorio3 = 0;
int aleatorio = 0;
int numero_de_eventos = 0;
bool evento_acontecendo = false;
int nu_meteoro = 1;
int estado = 0;
int menu = 0;
int ia = 1;
int j2 = 2;


int atualx = 0;
int atualy = 0;
int frame_fps = 0;

int raioAumentado = 0;

int ponto_1 = 0;
int ponto_2 = 0;


//classes
class Meteoro{
    public:
    float x, y;
    int atualx = 0;
    int atualy = 0;
    int frame_fps;
    bool ativo = false;
    int limite;

};
class Reto{
public:
    bool ativado;
    float x, y, xx, yy;
    int velocidade_x, velocidade_y;
    Color cor = WHITE;
};

class Bola{
public:
    float x, y;
    float velocidade_x, velocidade_y, raio;
    Color cor = WHITE;
    bool sombra = false;
    float distanciaInicial;
};


//fun�oes de tratamento de evento
void Mouse(Reto botao, int estadoReal){
    Vector2 mouse = GetMousePosition();
    if(CheckCollisionPointRec(mouse, Rectangle{botao.x, botao.y, botao.xx, botao.yy}) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        estado = estadoReal;
    }
}
void mov(Reto &reto){
    if(IsKeyDown(KEY_W)){
        reto.y -= reto.velocidade_y;
    }
    if(IsKeyDown(KEY_S)){
        reto.y += reto.velocidade_y;
    }
}

void Mov2(Reto &reto){
    if(IsKeyDown(KEY_UP)){
        reto.y -= reto.velocidade_y;
    }
    if(IsKeyDown(KEY_DOWN)){
        reto.y += reto.velocidade_y;
    }
}

//fun�oes de atualizacao de posicoes
void pingPong(Reto &reto, Reto &reto2, Reto &reto3, Reto &reto4, Bola &bola, Reto boom[5], Meteoro chuva[5]){
     if(CheckCollisionCircleRec(Vector2{bola.x, bola.y}, bola.raio, Rectangle{reto.x, reto.y, reto.xx, reto.yy})){
            if(bola.velocidade_x < 0){
               PlaySound(somBatida);
               bola.velocidade_y = 0.075 * (bola.y -(reto.y + (reto.yy /2)));
             bola.velocidade_x *= -1.08;
            }

        }
        if(bola.velocidade_x > 25){
                bola.velocidade_x = 25;
            }
            if(bola.velocidade_x < -25){
                bola.velocidade_x = -25;
            }
        if(CheckCollisionCircleRec(Vector2{bola.x, bola.y}, bola.raio, Rectangle{reto2.x, reto2.y, reto2.xx, reto2.yy})){
            if(bola.velocidade_x > 0){
               PlaySound(somBatida);

               bola.velocidade_y = 0.075 * (bola.y -(reto2.y + (reto2.yy /2)));
               bola.velocidade_x *= -1.08;
}
        }
       if(bola.velocidade_x > 25){
                bola.velocidade_x = 25;
            }
            if(bola.velocidade_x < -25){
                bola.velocidade_x = -25;
            }
        if(CheckCollisionCircleRec(Vector2{bola.x, bola.y}, bola.raio, Rectangle{reto3.x, reto3.y, reto3.xx, reto3.yy})){
            ponto_2 += 1;
            PlaySound(somPonto);
            bola.x = largura /2;
            bola.y = altura /2;
            bola.velocidade_x = 3;
            bola.velocidade_y = 3;
        }
        if(CheckCollisionCircleRec(Vector2{bola.x, bola.y}, bola.raio, Rectangle{reto4.x, reto4.y, reto4.xx, reto4.yy})){
            ponto_1 += 1;
            PlaySound(somPonto);
            bola.velocidade_x = 3;
            bola.velocidade_y = 3;
            bola.x = largura /2;
            bola.y = altura /2;
        }
        for(int i = 0; i < 5; i ++){
        if(((chuva[i].atualx >= 3 && chuva[i].atualy == 1)) && chuva[i].ativo == true &&  chuva[i].y >= chuva[i].limite && CheckCollisionCircleRec(Vector2{bola.x, bola.y}, bola.raio, Rectangle{boom[i].x, boom[i].y, boom[i].xx, boom[i].yy})){

        if(boom[i].ativado == false){
        float dx = bola.x - (boom[i].x /2);
        float dy = bola.y - (boom[i].y /2);
        float dh = sqrt((dx * dx) + (dy*dy));
        float potencia = 650 / (dh + 1);
        if(dh != 0){
            dx = dx / dh;
            dy = dy / dh;
            bola.velocidade_x = dx * potencia * 10;
            bola.velocidade_y = dy * potencia * 5;
            boom[i].ativado = true;
           if(bola.velocidade_x > 22){
                  bola.velocidade_x = 22;
              }
           if(bola.velocidade_x < -22){
                   bola.velocidade_x = -22;
              }
           if(bola.velocidade_y > 22){
                bola.velocidade_y = 22;
            }
            if(bola.velocidade_y < -22){
                bola.velocidade_y = -22;
            }
}
        }
        }
    }

        bola.x += bola.velocidade_x;
        bola.y += bola.velocidade_y;



        if(bola.y >= altura - bola.raio){
            PlaySound(somBatida2);
           if(bola.velocidade_y > 0){
            bola.velocidade_y *= -1;
           }
        }
        if(bola.y <= bola.raio){
            PlaySound(somBatida2);
          if(bola.velocidade_y < 0){
            bola.velocidade_y *= -1;
          }
        }
        if(reto.y <= 0 ){
            reto.y += reto.velocidade_y;
        }
        if(reto.y >= altura - reto.yy){
            reto.y -= reto.velocidade_y;
        }
        if(reto2.y <= 0 ){
            reto2.y += reto2.velocidade_y;
        }
        if(reto2.y >= altura - reto2.yy){
            reto2.y -= reto2.velocidade_y;
        }
        }
void Ia(Bola &bola, Reto &reto2){
        if(bola.y > reto2.y){
               reto2.y += reto2.velocidade_y;}
              if(bola.y < reto2.y){
              reto2.y -= reto2.velocidade_y;}
}

//funcoes de desenhar
Meteoro chuva[5];
Reto boom[5];
Reto botao2;
Reto botao;
Reto reto4;
Reto reto3;
Reto reto2;
Reto reto;
Reto sombra;
Reto sombra2;
void DrawlReto(Reto reto){
    DrawRectangle(reto.x, reto.y, reto.xx, reto.yy, reto.cor);
};

Bola bola;
Bola Sombra[5];

void Drawl(Bola bola){
    DrawCircle(bola.x, bola.y, bola.raio, bola.cor);
};

void meteoros(Texture2D meteoro,Meteoro chuva[5], Reto boom[5], Bola Sombra[5]) {

    for(int i = 0; i < 5; i++){
        if(chuva[i].ativo == false){
            chuva[i].atualx = 0;
            chuva[i].atualy = 0;
            chuva[i].frame_fps = 0;
            uniform_int_distribution<int> ale_largura(300, largura - 300);
            uniform_int_distribution<int> ale_altura(0, altura - 100);
            chuva[i].x = ale_largura(gen);
            chuva[i].limite = ale_altura(gen);
            chuva[i].y = -50;
            chuva[i].ativo = true;

            //sombra
            if(Sombra[i].sombra == false){
            Sombra[i].cor = preto;
            Sombra[i].x = chuva[i].x + (boom[i].xx /2) - 16;
            Sombra[i].y = chuva[i].limite + (boom[i].yy /2) + 26;
            Sombra[i].raio = 5;
            Sombra[i].sombra = true;
            Sombra[i].distanciaInicial = Sombra[i].y - chuva[i].y;
            }
        }
    }



    for(int i = 0; i <5; i++){
      if(chuva[i].ativo == true){

       chuva[i].frame_fps++;
    if(chuva[i].frame_fps == 20){
       chuva[i].frame_fps = 0;
       chuva[i].atualx++;



    }
    if(chuva[i].atualx > 4 ){

        chuva[i].atualx = 0;
        chuva[i].atualy++;
    }

    if(chuva[i].y < chuva[i].limite ){
        chuva[i].y += 6;

            float dS = Sombra[i].y - chuva[i].y;

            float proporcao = 1 - (dS/ Sombra[i].distanciaInicial);
            Sombra[i].cor.a = 10 + (proporcao * 180);
            Sombra[i].raio = 7 +(proporcao*20);
        if(Sombra[i].raio >= 25){
            Sombra[i].raio = 25;
        }
        if(chuva[i].atualy == 1 && chuva[i].atualx == 3){
            chuva[i].atualx = 3;
        }
        }
    if(chuva[i].y >= chuva[i].limite ){
        Sombra[i].sombra = false;
        chuva[i].y = chuva[i].limite;

      if(chuva[i].atualy < 1 || (chuva[i].atualy == 1 && chuva[i].atualx < 3)){
          chuva[i].atualx = 3;
          chuva[i].atualy = 1;
          if((chuva[i].atualx >= 3 && chuva[i].atualy == 1))
            DrawlReto(boom[i]);
          }
    }

        if(chuva[i].atualy > 2){
        chuva[i].atualx = 0;
        chuva[i].atualy = 0;
        chuva[i].ativo = false;

    }


    float imagem_origemx = chuva[i].atualx * boom[i].xx * 2.5;
    float imagem_origemy = chuva[i].atualy * boom[i].yy * 2.5;
    boom[i].x = chuva[i].x;
    boom[i].y = chuva[i].limite;
    if(Sombra[i].sombra == true){
    Drawl(Sombra[i]);
    }
    DrawTexturePro(meteoro, (Rectangle){imagem_origemx, imagem_origemy, boom[i].xx * 2.5 , boom[i].yy * 2.5}, (Rectangle){chuva[i].x, chuva[i].y, boom[i].xx , boom[i].yy }, {0,0}, 0, WHITE);

}
}
}

//////////////////main.
int main() {
    /////////////main.
    ///////////tela.

    SetConfigFlags(FLAG_WINDOW_HIDDEN | FLAG_WINDOW_RESIZABLE);
    InitWindow(largura, altura, "qualquer");

    int monitor = GetCurrentMonitor();
    largura = GetMonitorWidth(monitor);
    altura = GetMonitorHeight(monitor);
    SetWindowSize(largura, altura);
    SetWindowPosition(0, 0);
    ClearWindowState(FLAG_WINDOW_HIDDEN);   //////////tela.


    //texturas
    Texture2D meteoro = LoadTexture("meteoro.png.png");

     //sons
    InitAudioDevice();
    somPonto = LoadSound("ponto.wav.wav");
    somBatida = LoadSound("raquete.wav.wav");
    somBatida2 = LoadSound("soun.wav.wav");




    SetTargetFPS(150);

    botao.cor = azul;
    botao.xx = 400;
    botao.x = largura /2 - 200;
    botao.yy = 70;
    botao.y = altura /2 - 35 - 100;

    botao2.cor = vermelho;
    botao2.xx = 400;
    botao2.x = largura /2 - 200;
    botao2.yy = 70;
    botao2.y = altura /2 + 35 + 60;

    reto.cor = branco;
    reto.velocidade_x = 7;
    reto.velocidade_y = 7;
    reto.x = 50;
    reto.xx = 30;
    reto.y = altura /2 - 50;
    reto.yy = 100;

    reto2.cor = branco;
    reto2.velocidade_x = 7;
    reto2.velocidade_y = 7;
    reto2.xx = 30;
    reto2.x = largura - 30 - 50;
    reto2.y = altura /2 - 50;
    reto2.yy = 100;

    reto3.cor = BLACK;
    reto3.xx = 10;
    reto3.x = 0;
    reto3.y = 0;
    reto3.yy = altura;

    reto4.cor = BLACK;
    reto4.xx = 10;
    reto4.x = largura - 10;
    reto4.y = 0;
    reto4.yy = altura;

    bola.velocidade_x = 3;
    bola.velocidade_y = 3;
    bola.x = largura /2;
    bola.y = altura /2;
    bola.raio = 20;
    bola.cor = {245,245,245,255};
    for(int i = 0; i < 5; i++){
    boom[i].cor = nada;
    boom[i].velocidade_x = 7;
    boom[i].velocidade_y = 7;
    boom[i].x = 0;
    boom[i].xx = (float)meteoro.width /5 * 0.4;
    boom[i].y = 0;
    boom[i].yy = (float)meteoro.height /3 * 0.4;
    boom[i].ativado = false;
    }
    for(int i = 0; i < 5; i++){
        Sombra[i].cor = preto;
        Sombra[i].x = 0;
        Sombra[i].y = 0;
        Sombra[i].raio = 5;
        Sombra[i].sombra = false;

    }
    //loop
    while(!WindowShouldClose()){
        largura = GetScreenWidth();
        altura = GetScreenHeight();




        //tratamento de eventos
        if(estado != menu){
            frame++;
        }


        if(frame == evento_tempo ){
            aleatorio = evento(gen);
            numero_de_eventos = aleatorio;
            if(numero_de_eventos != 0){
            evento_acontecendo = true;
            frame_fps = 0;
            }
            else{
            evento_acontecendo = false;
            for(int i = 0; i <5; i ++){
                chuva[i].ativo = false;
            }

            }
            frame = 0;
        }
        if(estado == menu){
            Mouse(botao, ia);
            Mouse(botao2, j2);
        }
        if(estado == ia){

        mov(reto);

        }
        else if(estado == j2){
             mov(reto);
        Mov2(reto2);
        }
        //atualizacao de posicoes
        if(estado == ia){
            pingPong(reto, reto2, reto3, reto4, bola, boom, chuva);
            if(bola.x > largura / 2){
            Ia(bola, reto2);
        }
        }
        if(estado == j2){
            pingPong(reto, reto2, reto3, reto4, bola, boom, chuva);
        }
        //desenhar
        BeginDrawing();
        if(estado == menu){
            ClearBackground(BLACK);
            DrawlReto(botao);
            DrawlReto(botao2);
            DrawText(TextFormat("pong de mesa"),largura /4, 0, 100, WHITE);
            DrawText(TextFormat("modo ia"),((int)largura) /2 - 85, botao.y + 10, 45, WHITE);
            DrawText(TextFormat("modo 2 jogadores"),botao2.x + 25, botao2.y + 15, 40, WHITE);
        }
        else if(estado == ia || estado == j2){
            ClearBackground(verde);
        DrawRectangle(reto.x +8, reto.y +8, reto.xx, reto.yy, {0,0,0,100});
        DrawRectangle(reto2.x +8, reto2.y +8, reto2.xx, reto2.yy, {0,0,0,100});
        DrawlReto(reto);
        DrawlReto(reto2);

        DrawLine(largura / 2, 0, largura/2, altura, branco);
        DrawCircleLines(largura /2, altura/2, 300, branco);
        DrawlReto(reto3);
        DrawlReto(reto4);
        if(numero_de_eventos == nu_meteoro){
                meteoros(meteoro,chuva, boom, Sombra);
            }

        DrawText(TextFormat("%d",ponto_1),largura /3, 0, 80, WHITE);
        DrawText(TextFormat("%d",ponto_2),largura /1.5, 0, 80, WHITE);
         DrawCircle(bola.x + 8, bola.y + 8, bola.raio, {0, 0, 0, 100});
        Drawl(bola);}

        EndDrawing();
    }
    UnloadTexture(meteoro);
    UnloadSound(somBatida2);
    UnloadSound(somPonto);
    UnloadSound(somBatida);
    CloseAudioDevice();
    CloseWindow();
    return 0;
}
